...
class CornerMaskView(context: Context, private val corner: Int, private val radiusPx: Int) : View(context) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val cx = if (corner == 0 || corner == 2) r else w - r
        val cy = if (corner == 0 || corner == 1) r else h - r

        val fullSquare = Path().apply { addRect(0f, 0f, w, h, Path.Direction.CW) }
        val circle = Path().apply { addCircle(cx, cy, r, Path.Direction.CW) }
